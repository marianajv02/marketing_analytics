...
def server(input, output, session):
    ...
    @render.plot
    def histogram():
        sns.countplot(x='kmeans_9', data=df_train, order=df_train['kmeans_9'].value_counts().index)
    ...
    def pie_chart():
        ...
        plt.pie(counts, labels=counts.index, autopct='%1.1f%%', startangle=90, colors=sns.color_palette(), wedgeprops=dict(edgecolor='black'))
        plt.title('Distribution of customers accross segments')
        return plt.gcf()
    ...
    @render.plot  
    def test_histo():
        sns.countplot(x='cluster', data=df_New, order=df_New['cluster'].value_counts().index)
        plt.title('Distribution of customers accross segments')
        plt.xlabel('Cluster')
        plt.ylabel('Counts')
        return plt.gcf()
